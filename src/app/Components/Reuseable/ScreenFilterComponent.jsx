...
function ScreenFilterComponent() {
    const searchParams = useSearchParams();
    const query = searchParams.get('screenSize') || '';
        ...
        const fetchData = async () => {
            try {
                ...
                const filteredLaptops = laptops.filter(laptop =>
                    laptop?.screenSize?.toLowerCase().includes(query.toLowerCase())
                );
    ...
    return (
        <div>
            <h2 className='px-4 py-2'>Searched Results for Screen Size: {query}</h2>
...
export default ScreenFilterComponent;
