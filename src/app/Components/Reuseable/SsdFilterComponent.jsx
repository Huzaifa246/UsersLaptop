...
function SsdFilterComponent() {
    const searchParams = useSearchParams();
    const query = searchParams.get('ssd') || '';
        ...
        const fetchData = async () => {
            try {
                ...
                const filteredLaptops = laptops.filter(laptop =>
                    laptop?.ssd?.toLowerCase().includes(query.toLowerCase())
                );
    ...
    return (
        <div>
            <h2 className='px-4 py-2'>Searched Results for SSD: {query}</h2>
...
export default SsdFilterComponent;
